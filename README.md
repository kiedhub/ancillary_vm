...
Following is a description on how to install, configure and run ancillary_vm.
...
Clone the repo by issuing the following command on the host server cli and cd into the new directory:
```
...
Set the IP address, gateway and dns server configuraiton of the virtual machine management interface. Currently only static configuration is supported. If dhcp shall be used, then this can be changed via console loging and changing the networking of the virtual machin (using netplan). Derault configuration parameters are
```
vmMgmtIf="ens3"
...
Default username: casa
Default password: casa
